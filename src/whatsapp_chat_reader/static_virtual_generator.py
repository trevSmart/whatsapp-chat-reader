...
class StaticVirtualHTMLGenerator:
    def __init__(self):
        self.css_styles = """
        <style>
            body {
    ...
    def generate_chunks(self, messages: List[WhatsAppMessage], attachment_map: Dict[str, str], chunk_size: int = 100) -> List[Dict]:
        """Generate message chunks for virtual scrolling."""
        chunks = []
        ...
        for i in range(0, len(messages), chunk_size):
            ...
            chunks.append({
                ...
                'messages': chunk_data,
                'total_messages': len(chunk_data)
            })

        return chunks

    def generate_html(self, messages: List[WhatsAppMessage], attachment_map: Dict[str, str], chat_name: str = "WhatsApp Chat", output_dir: str = "") -> str:
        """Generate HTML with virtual scrolling using static files."""

        # Generate chunks
        print("Generant chunks de missatges...")
        chunks = self.generate_chunks(messages, attachment_map, chunk_size=100)
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunks_data.append({
                ...
                'chunk_index': i,
                'message_count': chunk['total_messages']
            })
    ...
    <script>
        class StaticVirtualChat {{
            constructor() {{
                ...
                this.currentChunk = 0;
                this.chunksData = {json.dumps(chunks_data)};
            ...
            async loadAllChunks() {{
                ...
                try {{
                    ...
                    const chunkPromises = this.chunksData.map(chunk =>
                        fetch(chunk.filename).then(response => response.json())
                    );
                    ...
                    chunks.forEach(chunk => {{
                        this.allMessages.push(...chunk.messages);
                    }});
                    ...
                    this.showError('Error carregant els missatges. Assegura\\'t que tots els fitxers JSON estan disponibles.');
                }} finally {{
